...
const StyledSubmitButton = styled.button<{ $secondary?: boolean }>`
  ...
  background: ${({ $secondary }) =>
    $secondary ? '#85fff0' : 'linear-gradient(90deg, #4fc5bf 0%, #2cc2a9 100%)'};
...
export default function UserFriendsContent() {
  ...
  const submitReferralCode = useGlobalStore((state) => state.submitReferralCode)
  ...
  const isReferralLocked = useMemo(() => Boolean(referredByCode), [referredByCode])
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Не удалось активировать код'
      ...
      <StyledReferralBlock>
        <div>
          ...
          <StyledCodeRow>
            ...
            <StyledCopyButton onClick={handleCopy} disabled={!referralCode} type="button">
        ...
        <StyledForm onSubmit={handleSubmit}>
          ...
          <StyledInput
            ...
            maxLength={16}
          />
          <StyledSubmitButton type="submit" disabled={isReferralLocked || isSubmitting}>
