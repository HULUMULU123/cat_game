...
type AdsgramHookResult = {
  ...
  error: string | null;
  isLoading: boolean;
  requestAssignment: (placementId?: string) => Promise<AdsgramAssignmentResponse>;
  confirmAssignment: (assignmentId: string) => Promise<AdsgramAssignmentResponse>;
...
const useAdsgramAd = (): AdsgramHookResult => {
  ...
  const [assignment, setAssignment] = useState<AdsgramAssignmentResponse | null>(
    null,
  );
  ...
  const requestAssignment = useCallback<
    AdsgramHookResult["requestAssignment"]
  >(
    async (placementId) => {
      try {
        requireAuth();
  ...
  const confirmAssignment = useCallback<
    AdsgramHookResult["confirmAssignment"]
  >(
    async (assignmentId) => {
      try {
        requireAuth();
