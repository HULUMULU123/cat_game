...
def _make_unusable_password_direct(user) -> None:
    """
    Универсально делает пароль непригодным для входа в миграции,
    не полагаясь на методы модели.
    """
    # В стандартной и кастомных моделях поле обычно называется 'password'
    if hasattr(user, "password"):
        user.password = make_password(None)  # ставит '!' префикс
    # если по какой-то причине поля нет — просто пропустим


def seed_initial_data(apps, schema_editor) -> None:
    # Берём «историческую» модель пользователя из AUTH_USER_MODEL
    ...
    if not getattr(user, "first_name", None):
        ...
        user.last_name = "User"
        _make_unusable_password_direct(user)
        user.save()
    ...
    for entry_data in leaderboard_data:
        ...
        if entry_data["position"] != 4:
            ...
            if not getattr(player, "first_name", None):
                player.first_name = f"Player {entry_data['position']}"
                _make_unusable_password_direct(player)
                player.save()
